...
export default function Home() {
  ...
  // scanSource tracks which mode produced the current results
  ...
  // Capture mode at scan-start so the progress messages stay consistent
  ...
  // Fetch total scan count for social proof (non-blocking)
  useEffect(() => {
    ;(async () => {
      try {
  ...
  /** Strip null bytes and control chars client-side before sending to any API. */
  ...
  async function handleUpload(files: File[]) {
    ...
    try {
      // Parse all transcripts in parallel
      ...
      // Deduplicate by EIN across all files; prefer entry with a real EIN over null
  ...
  async function handleW2Scan() {
    ...
    try {
      ...
      // Flatten and deduplicate by EIN (prefer entries with a real EIN over null)
      const seen = new Map<string, { employer_name: string; ein: string | null; has_redacted_ein?: boolean; years_employed: string }>()
      for (const list of parseResults) {
        for (const emp of list) {
          const key = emp.ein ?? emp.employer_name.toLowerCase().replace(/\s+/g, ' ').trim()
      ...
      <nav className="sticky top-0 z-50 border-b border-zinc-800/60 bg-zinc-950/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link
              href="/security"
              className="flex items-center gap-1.5 rounded-full border border-green-500/20 bg-green-500/5 px-3 py-1.5 text-xs text-green-400 transition-colors hover:bg-green-500/10"
            >
            ...
            {(['upload', 'scanning', 'results'] as const).map((s, i) => {
              ...
              return (
                <div key={s} className="flex items-center">
                  {i > 0 && (
                    <div className={`h-px w-10 sm:w-16 mx-1 transition-colors ${isDone || isActive ? 'bg-amber-500/50' : 'bg-zinc-800'}`} />
                  )}
        ...
        {step === 'upload' && (
          <div className="mx-auto max-w-2xl px-6 py-16 flex flex-col gap-10">
            ...
            <div className="flex flex-col items-center text-center gap-5">
              ...
              {/* Headline */}
              <h1
                className="text-5xl leading-[1.1] tracking-tight text-zinc-100"
              ...
              <p className="max-w-md text-base text-zinc-400 leading-relaxed">
                ...
                you&apos;ve worked for against the DOL database to surface unclaimed 401(k) accounts.
              </p>

              {/* Trust pills */}
              <div className="flex flex-wrap justify-center gap-3">
                {['No account needed', 'Never stored', 'Takes 30 seconds'].map((t) => (
                  <span key={t} className="flex items-center gap-1.5 rounded-full bg-zinc-900 border border-zinc-800 px-3 py-1 text-xs text-zinc-400">
                ...
                <div className="flex items-center gap-2 rounded-full border border-zinc-800 bg-zinc-900/80 px-4 py-2">
                  <div className="flex -space-x-1">
                    {['bg-amber-500', 'bg-orange-500', 'bg-yellow-500'].map((c, i) => (
                      <div key={i} className={`h-5 w-5 rounded-full ${c} ring-2 ring-zinc-900 flex items-center justify-center text-[9px] font-bold text-white`}>
                        {String.fromCharCode(65 + i)}
                      </div>
                    ))}
                  </div>
                  <span className="text-xs text-zinc-400">
                    <span className="font-semibold text-zinc-200">{scanCount.toLocaleString()}</span> scans completed
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              >
                <div className="flex items-center gap-2.5">
                  <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ring-1 transition-colors ${
                    scanMode === 'linkedin' ? 'bg-amber-500/15 ring-amber-500/30' : 'bg-zinc-800 ring-zinc-700'
                  }`}>
                    <Coins className={`h-4 w-4 ${scanMode === 'linkedin' ? 'text-amber-400' : 'text-zinc-500'}`} />
                  </div>
                  <span className="text-sm font-semibold text-zinc-100">Quick Scan</span>
                </div>
              ...
              >
                <div className="flex items-center gap-2.5">
                  <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ring-1 transition-colors ${
                    scanMode === 'w2' ? 'bg-amber-500/15 ring-amber-500/30' : 'bg-zinc-800 ring-zinc-700'
                  }`}>
                    <Receipt className={`h-4 w-4 ${scanMode === 'w2' ? 'text-amber-400' : 'text-zinc-500'}`} />
                  </div>
                  <span className="text-sm font-semibold text-zinc-100">W-2 Upload</span>
                </div>
              ...
              >
                <div className="flex items-center gap-2.5">
                  <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ring-1 transition-colors ${
                    scanMode === 'irs' ? 'bg-amber-500/15 ring-amber-500/30' : 'bg-zinc-800 ring-zinc-700'
                  }`}>
                    <FileText className={`h-4 w-4 ${scanMode === 'irs' ? 'text-amber-400' : 'text-zinc-500'}`} />
                  </div>
                  <span className="text-sm font-semibold text-zinc-100">IRS Transcript</span>
                </div>
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6 flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  className={`
                    rounded-xl border bg-zinc-950 px-4 py-3 text-sm text-zinc-100 placeholder-zinc-600
                    outline-none transition-all duration-200
                    focus:border-amber-500/60 focus:ring-2 focus:ring-amber-500/10
                    ${emailError ? 'border-red-500/60' : 'border-zinc-700/60'}
                  `}
                />
              ...
              {/* SSN */}
              <div className="flex flex-col gap-1.5">
                <label htmlFor="ssn" className="text-xs font-medium text-zinc-400 uppercase tracking-wider">
                  SSN <span className="normal-case text-zinc-600">(optional — improves accuracy)</span>
                </label>
                <input
                  id="ssn"
                  type="text"
                  ...
                  maxLength={11}
                  className="rounded-xl border border-zinc-700/60 bg-zinc-950 px-4 py-3 text-sm text-zinc-100 placeholder-zinc-600 outline-none transition-all duration-200 focus:border-amber-500/60 focus:ring-2 focus:ring-amber-500/10"
                />
              ...
              {scanMode === 'linkedin' && (
                <div className="flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-zinc-600">{manualEmployers.length}/{MAX_ROWS}</span>
                  </div>

                  {/* Column headers */}
                  <div className="grid grid-cols-[1fr_80px_80px_28px] gap-2 px-0.5">
                    <span className="text-[10px] font-medium text-zinc-600 uppercase tracking-wider">Company Name</span>
                    <span className="text-[10px] font-medium text-zinc-600 uppercase tracking-wider">From</span>
                    <span className="text-[10px] font-medium text-zinc-600 uppercase tracking-wider">To</span>
                    <span />
                  </div>
                    ...
                    {manualEmployers.map((emp, i) => (
                      <div key={i} className="grid grid-cols-[1fr_80px_80px_28px] gap-2 items-center">
                        <input
                          ...
                          className="rounded-lg border border-zinc-700/60 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 placeholder-zinc-600 outline-none transition-all focus:border-amber-500/50 focus:ring-1 focus:ring-amber-500/10"
                        />
                        <input
                          type="text"
                        ...
                        <button
                          ...
                          className="flex h-7 w-7 items-center justify-center rounded-lg text-zinc-600 transition-colors hover:bg-zinc-800 hover:text-zinc-400 disabled:opacity-30 disabled:cursor-not-allowed"
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <button
                      ...
                      className="flex items-center gap-1.5 text-xs text-zinc-500 transition-colors hover:text-amber-400 disabled:opacity-30 disabled:cursor-not-allowed"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 px-4 py-2.5 text-sm font-medium text-white shadow-md shadow-amber-500/20 transition-opacity hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      Search Employers
              ...
              {scanMode === 'w2' && (
                <div className="flex flex-col gap-3">
                  ...
                  <label
                    htmlFor="w2-input"
                    className="group flex flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-zinc-700 bg-zinc-950 p-8 cursor-pointer transition-all hover:border-amber-500/60 hover:bg-amber-500/[0.02]"
                  >
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-zinc-800 ring-1 ring-zinc-700 transition-colors group-hover:bg-amber-500/10 group-hover:ring-amber-500/30">
                      <UploadCloud className="h-5 w-5 text-zinc-500 transition-colors group-hover:text-amber-400" />
                    </div>
                    <div className="text-center">
                      <p className="text-sm font-medium text-zinc-200">Click to add W-2 PDFs</p>
                      <p className="text-xs text-zinc-500 mt-0.5">PDF format · multiple files supported</p>
                    </div>
                    <input
                      id="w2-input"
                      type="file"
                    ...
                    <div className="flex flex-col gap-1.5">
                      {w2Files.map((file, i) => (
                        ...
                        >
                          ...
                          <span className="flex-1 min-w-0 truncate text-sm text-zinc-200">{file.name}</span>
                          ...
                          <button
                            ...
                            className="shrink-0 flex h-6 w-6 items-center justify-center rounded-md text-zinc-600 transition-colors hover:bg-zinc-800 hover:text-zinc-400"
            ...
            {(() => {
              ...
              return (
                <div className="flex flex-col gap-2">
                  <h3 className="text-xs font-semibold uppercase tracking-widest text-zinc-600 px-1">Common questions</h3>
                    ...
                    {faqs.map((faq, i) => (
                      <div key={i}>
                        ...
                        >
                          ...
                          <ChevronDown className={`h-4 w-4 shrink-0 text-zinc-500 transition-transform duration-200 ${openFaq === i ? 'rotate-180' : ''}`} />
                        </button>
                        {openFaq === i && (
          ...
          <div className="flex min-h-[calc(100vh-65px)] flex-col items-center justify-center px-6 py-20 gap-10">
            ...
            <div className="relative flex items-center justify-center">
              ...
              <div className="absolute h-36 w-36 animate-radar rounded-full"
                style={{
                  background: 'conic-gradient(from 0deg, transparent 75%, rgba(245,158,11,0.6) 100%)',
                  borderRadius: '50%',
                }}
              />
              ...
              {/* Center */}
              <div className="relative flex h-16 w-16 items-center justify-center rounded-full bg-zinc-900 border border-zinc-800 shadow-xl">
            ...
            <div className="flex flex-col items-center gap-3">
              <p className="text-lg font-medium text-zinc-100 text-center min-h-[1.75rem] transition-all">
            ...
            <div className="mx-auto max-w-2xl px-6 py-12 flex flex-col gap-8">
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  ...
                  <button
                    onClick={handleShare}
                    className="flex items-center gap-1.5 rounded-xl border border-zinc-700/60 bg-zinc-900 px-3 py-2 text-xs font-medium text-zinc-400 transition-colors hover:border-amber-500/40 hover:text-amber-400"
                  >
                    {shareConfirmed
                      ? <><CheckCircle2 className="h-3.5 w-3.5 text-green-400" /><span className="text-green-400">Copied!</span></>
              ...
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900 overflow-hidden">
                <div className="px-5 py-4 border-b border-zinc-800/60">
                  <h3 className="text-sm font-semibold text-zinc-100">What&apos;s next?</h3>
                </div>
                {accountCount > 0 ? (
                  <div className="flex flex-col gap-3 px-5 py-4">
                      ...
                      <div key={i} className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-zinc-100">
                            Contact the plan administrator for <span className="text-amber-400">{name}</span>
                          </p>
                    ...
                    <div className="mt-1 rounded-xl border border-zinc-800 bg-zinc-950/60 px-4 py-3">
                      <p className="text-xs text-zinc-500 leading-relaxed">
                        If a plan has been terminated, file a claim at{' '}
                        <span className="text-zinc-400">dol.gov</span> (Abandoned Plan Program) or{' '}
                        <span className="text-zinc-400">pbgc.gov</span> (Missing Participants Program).
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="flex flex-col gap-3 px-5 py-4">
                    ...
                    <p className="text-sm text-zinc-400 leading-relaxed">
                      <span className="font-medium text-zinc-200">Try uploading individual W-2s</span> for a more accurate per-employer search using verified EINs directly from your tax records.
                    </p>
                    <button
                      onClick={() => resetToUpload('w2')}
                      className="self-start flex items-center gap-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 px-4 py-2.5 text-sm font-medium text-white shadow-md shadow-amber-500/20 transition-opacity hover:opacity-90"
                    >
                      Try W-2 Upload
                      <ArrowRight className="h-3.5 w-3.5" />
                    </button>
              ...
              {/* ── Email capture (shown when no email was entered) ── */}
              {!email && notifyStatus !== 'done' && (
                <div className="rounded-2xl border border-zinc-800 bg-zinc-900 px-5 py-5 flex flex-col gap-3">
                  <div className="flex items-center gap-2.5">
                    <Bell className="h-4 w-4 text-amber-400" />
                    <p className="text-sm font-semibold text-zinc-100">Get notified when we add more databases</p>
                  </div>
                  <p className="text-xs text-zinc-500 leading-relaxed">
                  ...
                  <form onSubmit={handleNotifySubmit} className="flex gap-2">
                    <input
                      ...
                      placeholder="you@example.com"
                      className="flex-1 rounded-xl border border-zinc-700/60 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 placeholder-zinc-600 outline-none focus:border-amber-500/50 focus:ring-1 focus:ring-amber-500/10"
                    />
                    <button
                      type="submit"
                      disabled={notifyStatus === 'sending'}
                      className="flex items-center gap-1.5 rounded-xl bg-amber-500/15 border border-amber-500/30 px-4 py-2 text-sm font-medium text-amber-400 transition-colors hover:bg-amber-500/20 disabled:opacity-50"
                    >
              ...
              {!email && notifyStatus === 'done' && (
                <div className="flex items-center gap-2.5 rounded-2xl border border-green-500/20 bg-green-500/5 px-5 py-4">
                  <CheckCircle2 className="h-4 w-4 text-green-400 shrink-0" />
                  <p className="text-sm text-green-400">You&apos;re on the list — we&apos;ll email you when new databases are added.</p>
                </div>
              )}

              <button
                onClick={() => resetToUpload()}
                className="mx-auto flex items-center gap-2 text-sm text-zinc-500 hover:text-amber-400 transition-colors group"
              >
